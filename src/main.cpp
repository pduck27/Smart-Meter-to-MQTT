...
// Wifi procedures
unsigned long wifiLostSince = 0;     // 0 = WLAN ist verbunden
unsigned long lastWifiRetry = 0;
const unsigned long WIFI_RETRY_MS = 30000;      // alle 30 s neu versuchen
const unsigned long WIFI_RESTART_MS = 300000;   // nach 5 min ohne WLAN: Neustart
...
void wifiInitialization()
{
  ...
  while (WiFi.status() != WL_CONNECTED)
  {
    ...
    delay(500);
    if (millis() - start > 30000) // nicht ewig warten
    {
...
// Wird in jedem loop() aufgerufen und blockiert nie
void checkWifi()
{
  ...
  if (now - wifiLostSince > WIFI_RESTART_MS)
  {
    Serial.println("WiFi gone too long, restarting.");
    ESP.restart();
  }
}

// MQTT procedures
// Gibt true zurueck, wenn verbunden. Hoechstens 3 Versuche, dann aufgeben.
bool connectToMQTTBroker()
{
  ...
  if (WiFi.status() != WL_CONNECTED)
    return false; // ohne WLAN gar nicht erst versuchen

  for (int i = 0; i < 3; i++)
  {
    if (client.connect("ESP32Smartmeter", mqttUser, mqttPassword))
    {
...
void sendMQTTPayload()
{
  ...
  {
    Serial.println("No MQTT connection, skipping this value.");
    return;
  }
...
void mqttInitialization()
{
  ...
  client.setSocketTimeout(5); // Sekunden, statt 15 s Standard
  connectToMQTTBroker();
}
...
void identify(String content)
{
  ...
  if (foundSomething >= 3)
  {
    ...
    delay(1000);
    digitalWrite(LED_PIN, LOW);
    nextCheckMillis = millis() + 15000; // Change nummer in millisceonds for other delay
  }
}
...
void loop()
{
  ...
  checkWifi();
  client.loop(); // haelt die MQTT-Verbindung am Leben
